...
struct CustomAlertQuiz: View {
    @Binding var shown: Bool
    @Binding var closureA: AlertAction2?
    var oneBtn: Bool
    ...
    @EnvironmentObject var quizManager1: AQuizManagerVM
        var body: some View {
            NavigationLink(destination: SignIn(), isActive: $showNextPage, label: {EmptyView()} )
            ZStack{
               ...
               // Divider()
                HStack {
                    ...
                    Button(btn1) {
                        ...
                        quizManager1.updateStatus(status: "accepted")
                        }else{
                            //////////delete user
                            quizManager1.deleteUser()
                        }
                        showNextPage = true
